<template>
  <div class="container-body">
    ...
    >
      ...
      <el-table-column prop="dd" label="风险实施状态" align="left">
      ...
      <el-table-column label="操作" width="200px" align="left" fixed="right">
        <template slot-scope="scope">
          ...
          >查看风险实施</el-button>
          <el-button v-else size="mini" type="warning" @click="measureFill(scope.row)">填写风险实施</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--分页组件-->
  ...
  mounted() {
    ...
    for (let i = 0; i < 5; i++) {
      this.data.push({
        ...
        dd: i % 2 == 0 ? "风险实施确认通过" : "已完成风险实施制订待确认"
      });
    }
  },
  methods: {
    ...
    measureFill(row) {
      let _this = this.$refs.implementationFillIndex;
      _this.dialog = true;
    }
  }
};
