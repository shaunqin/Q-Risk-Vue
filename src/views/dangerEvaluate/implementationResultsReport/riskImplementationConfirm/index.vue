    ...
    >
      ...
      <el-table-column label="操作" width="200px" align="left" fixed="right">
        <template slot-scope="scope">
          ...
          >查看风险措施</el-button>
          <el-button v-else size="mini" type="warning" @click="measureConfirm(scope.row)">确认风险措施</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--分页组件-->
...
export default {
  components: { implementationDetail, confirmDialog },
  mixins: [initData],
  data() {
    return {
      isSuperAdmin: false,
  ...
  mounted() {
    ...
    for (let i = 0; i < 5; i++) {
      this.data.push({
        ...
        dd: i % 2 == 0 ? "风险措施确认通过" : "已完成风险措施制订待我确认"
      });
    }
  },
  methods: {
    ...
    measureConfirm(row) {
      let _this = this.$refs.confirmDialog;
      _this.dialog = true;
    }
  }
};
